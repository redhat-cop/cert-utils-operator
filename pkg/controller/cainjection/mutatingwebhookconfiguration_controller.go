...
const controllerNamemwh = "mutatingwebhookconfiguration-controller"
...
func newMutatingReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &ReconcileMutatingWebhookConfiguration{
		ReconcilerBase: outils.NewReconcilerBase(mgr.GetClient(), mgr.GetScheme(), mgr.GetConfig(), mgr.GetEventRecorderFor(controllerNamemwh)),
	}
}
...
func addMutating(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	c, err := controller.New(controllerNamemwh, mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}
	...
	}}, &handler.EnqueueRequestForObject{}, isAnnotatedMutatingWebhookConfiguration)
	if err != nil {
		return err
	}
